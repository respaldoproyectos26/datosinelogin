...
// (opcional) permisos
#require_permission('ver_formulario');

// Página: Nuevo registro (solo formulario)
$pageTitle  = 'Nuevo registro RG';
$pageHeader = 'Nuevo registro RG';
...
// Script RC
$scriptPath = FORM_DIR . '/js/script.js';
$scriptVersion = file_exists($scriptPath) ? filemtime($scriptPath) : time();
$customScripts[] =
  '<script src="'.FORM_DATOS1_URL.'/js/script.js?v='.$scriptVersion.'" defer></script>';
// Tom-select
...
$FORM_FLAGS = [
  'show_dtto_fed'        => false,
  'show_dtto_loc'        => false,
  'show_municipio_form'  => false, // ajusta si RG usa municipio

  'show_tipo_nombramiento'=> false, // 🔥 se quita tipo_form_ui
  'tipo_propietario'     => false,
  'tipo_suplente'        => false,

  'show_num_ruta'        => true,
  'show_secciones_ruta'  => true,
...
<?php
$file = path_join(FILT_DATOS1_URL,'index.php');
$version = file_exists($file) ? filemtime($file) : time();
?>
<div class="d-flex align-items-center w-100 justify-content-center mb-3">
  <a href="../filtrodatos/" class="btn btn-warning fw-bolder">Cancelar</a>
  <!--<a href="../filtrodatos/?= <?=  $version ?>" class="btn btn-warning fw-bolder">Cancelar</a>-->
</div>
