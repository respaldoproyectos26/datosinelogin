...
// (opcional) permisos
#require_permission('ver_formDistLoc');

// Página: Nuevo registro (solo formulario)
$pageTitle  = 'Nuevo registro Rep. Distrital Local';
$pageHeader = 'Registro Rep. Distrital Local';
...
// Script Dist. Loc.
$scriptPath = FORM_DISTLOC_DIR . '/js/script.js';
$scriptVersion = file_exists($scriptPath) ? filemtime($scriptPath) : time();
$customScripts[] =
  '<script src="'.FORM_DISTLOC_URL.'/js/script.js?v='.$scriptVersion.'" defer></script>';
// Tom-select
...
$FORM_FLAGS = [
  'show_dtto_fed'        => false,
  'show_dtto_loc'        => true,   // etiqueta Dtto. Local, name sigue distfed_form
  'show_municipio_form'  => false,

  'show_tipo_nombramiento'=> false,
...
<?php
$file = path_join(FILT_MUNICIPAL_URL,'index.php');
$version = file_exists($file) ? filemtime($file) : time();
?>
<div class="d-flex align-items-center w-100 justify-content-center mb-3">
  <a href="../filtDistLoc/" class="btn btn-warning fw-bolder">Cancelar</a>
  <!--<a href="../filtDistLoc/?= <?=  $version ?>" class="btn btn-warning fw-bolder">Cancelar</a>-->
</div>
