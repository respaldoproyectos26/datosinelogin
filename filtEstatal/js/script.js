...
$(function () {
  ...
  // Escape mínimo para HTML (evita romper selects/atributos si hay caracteres raros)
  ...
  // Cargar filtros (en paralelo) y luego inicializar tabla
  $.when(
  ...
  // - Si existe modal (#modalCrear) lo abre
  ...
  $.fn.dataTable.ext.buttons.crear = {
    ...
    action: () => {
      ...
      if (modalEl && formEl) {
        ...
        window.location.href = '../formEstatal/index.php';
      }
    }
  };
  ...
  function initTabla() {
    ...
    // Loader (solo una vez, sobre la instancia real)
  ...
  $('#productsTable').on('click', '.editar-btn', function () {
    ...
    window.location.href = `../formEstatal/index.php?id=${encodeURIComponent(id)}`;
  });
  ...
  // Cascada Estado → Municipio → Sección (solo si existen esos selects en ESTA vista)
  const $edo = $('#idedo');
  ...
  // Si tienes modal de creación y formCrear en esta vista, aquí NO te recomiendo
  // meter validaciones de inputs ni normalización; eso va en el JS del formulario (formEstatal).
});
