...
if ($filtro === 'seccion') {
  $sql = "SELECT DISTINCT seccion FROM registrorepdistfed WHERE seccion IS NOT NULL 
          ORDER BY CAST(seccion AS UNSIGNED), seccion";
} else {
  $sql = "SELECT DISTINCT $filtro FROM registrorepdistfed WHERE $filtro IS NOT NULL ORDER BY $filtro ASC";
}
