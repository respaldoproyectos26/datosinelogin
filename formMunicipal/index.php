...
// (opcional) permisos
#require_permission('ver_formMunicipal');

// Página: Nuevo registro (solo formulario)
$pageTitle  = 'Nuevo registro Representante Municipal';
$pageHeader = 'Registro Representante Municipal';
...
// Script Municipal
$scriptPath = FORM_MUNICIPAL_DIR . '/js/script.js';
$scriptVersion = file_exists($scriptPath) ? filemtime($scriptPath) : time();
$customScripts[] =
  '<script src="'.FORM_MUNICIPAL_URL.'/js/script.js?v='.$scriptVersion.'" defer></script>';
// Tom-select
...
<?php
$file = path_join(FILTRO_MUN_DIR,'index.php');
$version = file_exists($file) ? filemtime($file) : time();
?>
<div class="d-flex align-items-center w-100 justify-content-center mb-3">
  <a href="../filtMunicipal/" class="btn btn-warning fw-bolder">Cancelar</a>
  <!--<a href="../filtMunicipal/?= <?=  $version ?>" class="btn btn-warning fw-bolder">Cancelar</a>-->
</div>
